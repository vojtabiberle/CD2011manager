<div class="students reg form register">
    ...
    <?php
        if(!isset($showform))
        {
            echo '<fieldset><legend>Zadej registrační kód</legend>';
                echo $this->Form->create('Student', array('url' => '/check_in/smsreg'));
                echo $this->Form->input('regcode', array('label' => ''));
                echo $this->Form->end('Zkontroluj');
            echo '</fieldset>';
        }
        else
        {
    ...
    <fieldset>
        ...
        //unset($this->data['User']['passwordCheck']);
            ...
            echo $this->Form->hidden('checkform', array('value' => '1'));

            if($dnu == 1)
            {
                echo $this->Form->radio('den', array('1' => 'první', '2' => 'druhý'));
            }
